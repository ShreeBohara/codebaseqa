...
import { ApiError, PlatformConfig, api, Repository } from '@/lib/api-client';
import {
  Github,
  Loader2,
  Trash2,
...
function RepoCard({ repo, onDelete, allowDelete }: RepoCardProps) {
  ...
  const statusConfig: Record<string, { color: string; label: string; pulse?: boolean }> = {
    completed: { color: 'text-emerald-400', label: 'Ready' },
    failed: { color: 'text-red-400', label: 'Failed' },
    pending: { color: 'text-amber-400', label: 'Queue', pulse: true },
    cloning: { color: 'text-blue-400', label: 'Cloning', pulse: true },
    parsing: { color: 'text-indigo-400', label: 'Parsing', pulse: true },
    embedding: { color: 'text-purple-400', label: 'Embedding', pulse: true },
  };
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={`group relative bg-zinc-900/40 border border-white/5 rounded-3xl p-6 backdrop-blur-sm overflow-hidden transition-colors ${
        isReady ? 'hover:border-indigo-500/30' : 'opacity-80'
      }`}
    >
      <div className="absolute top-6 right-6 flex items-center gap-2">
        {status.pulse && (
          <span className="relative flex h-2 w-2">
            <span
              className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${status.color.replace('text-', 'bg-')}`}
            ></span>
      ...
      <div className="grid grid-cols-3 gap-2 mb-6">
        <Link
          ...
          className={`flex flex-col items-center justify-center gap-1.5 p-3 rounded-xl border border-white/5 transition-colors ${
            isReady
...
export function RepoList({ initialRepos, platformConfig }: RepoListProps) {
  const [repos, setRepos] = useState<Repository[]>(initialRepos);
  const [prefillUrl, setPrefillUrl] = useState<string>('');
  const [loadingDemo, setLoadingDemo] = useState(false);

  const isDemoMode = !!platformConfig?.demo_mode;
  const demoRepoId = platformConfig?.demo_repo_id || undefined;

  const visibleRepos = useMemo(() => {
    if (!isDemoMode) return repos;
    if (demoRepoId) {
      const featured = repos.filter((repo) => repo.id === demoRepoId);
      return featured.length > 0 ? featured : repos;
    }
    return repos;
  }, [repos, isDemoMode, demoRepoId]);
  ...
  useEffect(() => {
    const hasPending = visibleRepos.some((r) => ['pending', 'cloning', 'parsing', 'embedding'].includes(r.status));
  ...
  return (
    <div>
      ...
      {isDemoMode && (
        <div className="mb-8 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 px-5 py-4 text-sm text-indigo-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <p className="font-medium">Live demo: single featured repository</p>
            <p className="text-indigo-200/80">{platformConfig?.demo_repo_full_name || 'Featured repository'} is pre-indexed for speed and quality.</p>
          </div>
          <a
          ...
          >
            Use your own repo via self-host
          </a>
        </div>
      )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12">
            <div className="text-center mb-12">
              ...
              <h3 className="text-2xl font-bold text-white mb-3">
                {isDemoMode ? 'Loading Featured Demo' : 'Start Your Journey'}
              </h3>

              <p className="text-zinc-400 max-w-md mx-auto mb-6">
                {isDemoMode
                  ? 'This deployment is pinned to one pre-indexed repository. Click below to prepare it if needed.'
                  : 'Import any public GitHub repository to explore its architecture, chat with the code, and generate personalized learning paths.'}
              </p>
              ...
              >
                {loadingDemo ? (
                  ...
                  <>
                    <Sparkles size={18} />
                    {isDemoMode ? 'Prepare Featured Demo' : 'Load Demo Repository'}
                  </>
                )}
              </button>
              <p className="text-xs text-zinc-500 mt-2">{isDemoMode ? 'No repo import is required for live demo mode' : 'Try with a pre-configured example project'}</p>
            </div>
